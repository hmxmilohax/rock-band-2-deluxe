...
if check_git_updated(repo_url="https://github.com/hmxmilohax/rock-band-2-deluxe", repo_root_path=root_dir):
    if not root_dir.joinpath("_build/xbox/gen/patch_xbox_0.ark").is_file():
        print("Rock Band 2 Deluxe ark not found, building it now...")
        rb2dx_res = build_patch_ark(True)
else:
    print("Local repo out of date, pulling and building an updated Rock Band 2 Deluxe ark now...")
    rb2dx_res = build_patch_ark(True)
...
# if rb2dx_res:
#     print("Ready to run Rock Band 2 Deluxe in Xenia.")
